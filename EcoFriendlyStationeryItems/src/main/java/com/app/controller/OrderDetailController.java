...
public class OrderDetailController {
	...
	String showCustomerDeliveredOrders(HttpServletRequest request, Order order, HttpSession session, Model model) {
		try {
			session = request.getSession(false);
			String customerEmail = (String) session.getAttribute("email");
			if (customerEmail == null) {
			...
			List<OrderDetail> ordersList = orderDetailService.getAllDeliveredOrdersOrderId(orderIds, customerEmail, orderStatus);
			log.info("orderIds size = "+orderIds.size());
			log.info("ordersList size = "+ordersList.size());
			model.addAttribute("orders", ordersList);
			return "my-history";
